...
function EditableSpan({
  ...
  // FIX: Force opaque background if the text differs from original to prevent visual overlap
  const isActuallyEdited = edited || value !== originalText;
  ...
  const commit = () => {
    ...
    if (v !== value || isActuallyEdited) onChange(v);
  };
  ...
  return (
    <span
      ...
      className={
        "absolute cursor-text whitespace-pre rounded-sm px-0.5 leading-none caret-foreground outline-none transition hover:bg-white hover:text-foreground focus:bg-white focus:text-foreground focus:ring-2 focus:ring-primary/60 " +
        (isActuallyEdited
