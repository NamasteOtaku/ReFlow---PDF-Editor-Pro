...
export async function renderPageForOcr(
  ...
  targetDpi = 600, // FIX: Increased from 300 to 600 DPI for AI generated images
): Promise<{ canvas: HTMLCanvasElement; scale: number }> {
  // PDF base is 72 DPI. Force at least the requested DPI and
...
export async function ocrPage(opts: {
  ...
  const { canvas: hiCanvas, scale: ocrScale } = await renderPageForOcr(page, 600); // 600 DPI
  // Deskew
  ...
  const result = await Tesseract.recognize(deskewed, language, {
    ...
    // FIX: Force Tesseract into Sparse Text mode to handle scattered AI-generated text blocks
